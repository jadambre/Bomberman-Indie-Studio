...
- Archive: `Bomberman-Indie-Studio-v1.0.0-Windows-x64.zip`, 52,280,650 bytes, 148 entries, with a single `Bomberman-Indie-Studio-Windows` root folder.
- SHA-256: `58c5318d0a545b62d4f87ebe8c18ec0cfaafacbca5a0fc9de16bd3b4a5bfcb3c`.
- The package contains the executable, matching `_Data` directory, Unity DLLs, Mono runtime, D3D12 support, crash handler and an English launch guide. Obsolete players, project caches, local saves and logs are not included.
...
Release documentation was subsequently changed to English only. The archive README was replaced and the checksum regenerated. All 147 remaining archive entries are byte-identical to the previously verified package; the runtime startup check above still applies to the unchanged executable and game files.
